...
import { runV3GenerationJobsWorkerCycle } from '@/lib/services/gradPlanV3GenerationJobService';
...
async function runWorker(request: NextRequest) {
  ...
  const processed = await runV3GenerationJobsWorkerCycle(limit);
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to run v3 generation worker',
      },
      { status: 500 }
    );
  }
