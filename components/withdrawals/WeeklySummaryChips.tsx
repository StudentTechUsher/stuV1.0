...
export default function WeeklySummaryChips({ summary }: WeeklySummaryChipsProps) {
  ...
  return (
    <div className="flex flex-wrap gap-3">
      {/* Total chip - Primary green with black text */}
      <div className="inline-flex items-center gap-2 rounded-lg bg-[var(--primary)] px-4 py-2.5 shadow-sm">
        <svg className="h-4 w-4 text-[#0A0A0A]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
      ...
      {topMajors.map(([majorId, count]) => (
        <div
          key={majorId}
          className="inline-flex items-center gap-2 rounded-lg border border-[var(--border)] bg-white px-4 py-2.5 shadow-sm"
        >
          <div className="flex h-6 w-6 items-center justify-center rounded-md bg-[color-mix(in_srgb,var(--primary)_15%,white)]">
